...
def prompt_spec(rec: dict, port: dict, param: dict | None) -> dict:
    ...
    precondition would be taught as fact to the one developer least able to
    catch it.
    """
    from lexicon import KIND_CONSTRAINTS, PARAM_OVERRIDES, PROMPTS
    kind = port["port_kind"]
    base = dict(PROMPTS.get(kind, {}))
    key = (rec.get("name"), (param or {}).get("name") or port.get("label"))
    over = PARAM_OVERRIDES.get(key, {})
    spec = {**base, **over}
    constraint = over.get("constraint") or KIND_CONSTRAINTS.get(kind)
    if param and param.get("default_value"):
        spec.setdefault("example", param["default_value"])
    return {
        "input_kind": spec.get("input_kind")
                      or ("produced" if port["direction"] == "out" else "expression"),
...
def derive_ports(rec: dict, params: list[dict]) -> list[dict]:
    ...
    by_ord = {p["ordinal"]: p for p in params}
    for pt in ports:
        pids = pt.get("param_ids") or []
        param = by_ord.get(pids[0]) if pids else None
        pt.update(prompt_spec(rec, pt, param))
