...
def macro_concept(rec: dict):
    """Macros are named constants and markers; verb analysis does not fit."""
    from lexicon import MACRO_TERMS
    name = rec["name"]
    authored = MACRO_TERMS.get(name)
    slug_name = slug(name) or name.lower()
...
def canonical_term(rec: dict) -> tuple[str, str]:
    if rec.get("kind") == "macro":
        _k, _g, _a, _q, authored = macro_concept(rec)
        if authored:
            return authored[0], "declared"
        return f"{rec['name']}: a standard macro", "derived"
    return _canonical_term_impl(rec)


def _canonical_term_impl(rec: dict) -> tuple[str, str]:
...
def intent_candidate_keys(rec: dict, aparams: list[dict]) -> list:
    if rec.get("kind") == "macro":
        ikey, _g, _a, _q, authored = macro_concept(rec)
        out = [("canonical", ikey, 10.0), ("symbolic", rec["name"], 9.0)]
        for i, al in enumerate((authored[1] if authored else [])):
            out.append(("colloquial", al, 6.0 - i * 0.1))
            if i == 0:
                out.append(("question", f"how-do-i.{al}", 5.0))
        seen, uniq = set(), []
        for kt, k, w in out:
            if k and k.lower() not in seen:
                ...
                uniq.append((kt, k, w))
        return uniq
    return _intent_candidate_keys_impl(rec, aparams)


def _intent_candidate_keys_impl(rec: dict, aparams: list[dict]) -> list:
        ...
        for kt, key, w in e["_cands"]:
            ...
            if k.lower() in taken:
                ...
                for suffix in refine:
                    ...
                    k = f"{k}.v{n}"
            taken[k.lower()] = idx
            result[idx].append((kt, k, w))   # same _idx may appear twice
            if kt == "canonical":
                got_canonical = True
...
def prepare(recs: list[dict]) -> list[dict]:
    ...
    for r in recs:
        by_q[(r.get("lang", "cpp"), r["qualified_name"])].append(r)

    out = []
    for i, r in enumerate(recs):
        # roles may already be assigned by langs/<lang>/structure.py, which is
        # the authority; only fall back when running standalone.
        ap = r.get("_params") or annotate_params(r)
        r["_params"] = ap
        r["_idx"] = i
        r["_primacy"] = primacy_rank(r, ap)
        r["_disamb"] = disambiguator(ap)
        r["_shape"] = "-".join(p["role"] or "input" for p in ap) or None
        siblings = by_q[(r.get("lang", "cpp"), r["qualified_name"])]
